    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(cartStore).isEmpty) {
        ...
        ssrRenderList(unref(cartStore).items, (item) => {
          _push(`<div class="card p-4 flex gap-4"><div class="w-20 h-20 rounded-xl bg-slate-100 overflow-hidden flex-shrink-0"><img${ssrRenderAttr("src", item.gift?.images?.[0] || "/placeholder-gift.jpg")}${ssrRenderAttr("alt", item.gift?.name || "Gift")} class="w-full h-full object-cover"></div><div class="flex-1"><h3 class="font-heading font-semibold text-slate-900">${ssrInterpolate(item.gift?.name || "Unknown Gift")}</h3><p class="text-sm text-primary-600 font-medium">${ssrInterpolate(unref(formatNaira)(Number(item.gift?.discountPrice) || Number(item.gift?.price) || 0))}</p></div><div class="flex items-center gap-3"><div class="flex items-center border border-slate-200 rounded-lg"><button class="px-3 py-1 text-slate-600 hover:bg-slate-50">−</button><span class="px-3 py-1 font-medium text-sm">${ssrInterpolate(item.quantity)}</span><button class="px-3 py-1 text-slate-600 hover:bg-slate-50">+</button></div><button class="p-2 text-red-500 hover:bg-red-50 rounded-lg">`);
...
//# sourceMappingURL=cart-Cjrq_wQx.js.map
